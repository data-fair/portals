...
test.describe('card thumbnail on the server-rendered path', () => {
  ...
  // center goes through v-img, left through a background-image and its hidden probe:
  // the probe is in the server html with its src, so a fast failure fires its error
  // event before hydration attaches the listener and the mount check has to catch it
  for (const location of ['center', 'left'] as const) {
    test(`the first candidate is server-rendered and the cascade takes over on the client (${location})`, async ({ page, goToPortal, request }) => {
        ...
        config: {
          ...
          datasets: {
            card: {
              titleLinesCount: 1,
              showSummary: true,
              thumbnail: { show: true, location, crop: true, default: imageRef('default-1') }
            }
          }
        }
      })).data
