...
export type RequestPortal = Pick<Portal, '_id' | 'config'> & { draft: boolean }
...
export default defineEventHandler(async (event) => {
  ...
  const portal = await mongo.portals.findOne(
    ...
    { projection: { _id: 1, config: draftMatch ? undefined : 1, draftConfig: draftMatch ? 1 : undefined } }
  )
  if (!portal) throw createError({ status: 404, message: 'portal not found' })
  const requestPortal: RequestPortal = { _id: portal._id, config: draftMatch ? portal.draftConfig : portal.config, draft: !!draftMatch }
