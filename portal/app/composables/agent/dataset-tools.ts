...
export function useAgentDatasetTools (locale: Ref<string>) {
  ...
  useAgentTool({
    name: 'list_datasets',
    description: 'List datasets published on this portal with optional text search. Returns id, title, status, row count, and last update.',
    ...
    execute: async (params) => {
      ...
      const data = await localFetch<any>('/data-fair/api/v1/catalog/datasets', { query })
  ...
  useAgentTool({
    ...
    description: 'Get detailed metadata and column schema for a dataset. Returns title, description, status, visibility, topics, and full column schema with types.',
