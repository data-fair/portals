...
const datasetUrl = (id: string) => `/data-fair/api/v1/datasets/${encodeURIComponent(id)}`
...
export function useAgentDatasetDataTools (locale: Ref<string>) {
  ...
  useAgentTool({
    ...
    execute: async (params) => {
      const [dataset, linesData] = await Promise.all([
        localFetch<any>(datasetUrl(params.datasetId), { query: { select: 'schema,title' } }),
        localFetch<any>(`${datasetUrl(params.datasetId)}/lines`, { query: { size: '3' } })
      ])
  ...
  useAgentTool({
    ...
    execute: async (params) => {
      ...
      if (params.next) {
        ...
        data = await localFetch<any>(`${datasetUrl(params.datasetId)}/lines`, { query })
      }
  ...
  useAgentTool({
    ...
    execute: async (params) => {
      ...
      const data = await localFetch<any>(`${datasetUrl(params.datasetId)}/values_agg`, { query })
  ...
  useAgentTool({
    ...
    execute: async (params) => {
      ...
      const data = await localFetch<any>(`${datasetUrl(params.datasetId)}/metric_agg`, { query })
  ...
  useAgentTool({
    ...
    execute: async (params) => {
      ...
      const values = await localFetch<any>(
        `${datasetUrl(params.datasetId)}/values/${encodeURIComponent(params.fieldKey)}`,
        { query }
      )
  ...
  useAgentSubAgent({
    ...
    prompt: `You are a data analyst assistant for a data portal. You help users explore and understand datasets by querying their content.
